...
export default function StoreDashboard() {
  ...
  const orderStatusData = [
    { name: 'Completed', value: storeData.orderStatus.completed, color: '#10b981' },
    { name: 'Preparing', value: storeData.orderStatus.preparing, color: '#f59e0b' },
    { name: 'Pending', value: storeData.orderStatus.pending, color: '#3b82f6' },
    { name: 'Cancelled', value: storeData.orderStatus.cancelled, color: '#ef4444' }
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-4xl font-bold text-gray-900">{storeData.branchName}</h1>
      ...
      {isClosed && (
        <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-red-700">This branch is not operating today. Showing last available data.</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-green-50 to-emerald-50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">₱{storeData.todayStats.revenue.toLocaleString()}</div>
            {!isClosed && (
        ...
        <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-blue-50 to-cyan-50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">{storeData.todayStats.orders}</div>
            {!isClosed && (
              <div className="flex items-center gap-1.5 mt-2">
        ...
        <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-orange-50 to-red-50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">₱{storeData.todayStats.avgOrderValue}</div>
        ...
        <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-purple-50 to-pink-50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">{storeData.todayStats.customers}</div>
            <p className="text-xs text-gray-500 mt-2 font-medium">
                    ...
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{order.time} • {order.items} items</div>
                        </div>
                      </div>
                ...
                <div className="space-y-3">
                  {storeData.inventoryAlerts.map((alert, index) => (
                    <div key={index} className={`p-3 rounded-lg ${
                      alert.status === 'critical' ? 'bg-red-50 border border-red-200' : 'bg-orange-50 border border-orange-200'
                    }`}>
                      <div className="flex items-center justify-between">
                        <div>
